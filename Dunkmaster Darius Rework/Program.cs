...
namespace Dunkmaster_Darius_Rework
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Config Menu
            Config = new Menu("Dunkmaster Rework", "Dunkmaster",true);

            //SLx_Orbwalker
            var orbwalkerMenu = new Menu("Orbwalker", "SLx_Orbwalker");
        ...
        KeyBindType.Toggle)));
            ...
            Game.PrintChat("faNsTaaR : Dunkmaster Darius Rework");
        ...
        static void AfterAttack(Obj_AI_Base unit, Obj_AI_Base target)
        {
            ...
            if (target.IsMinion)
            {
                if (Config.Item("UseWL").GetValue<bool>() && W.IsReady() && xSLxOrbwalker.InAutoAttackRange(target) && xSLxOrbwalker.CurrentMode == xSLxOrbwalker.Mode.LaneClear)
                {
                    W.Cast();
        ...
        static void ExecuteKillsteal()
        {
            ...
            foreach (var target in ObjectManager.Get<Obj_AI_Hero>())
            {
                ...
                if (igniteSlot != SpellSlot.Unknown && ObjectManager.Player.Spellbook.CanUseSpell(igniteSlot) == SpellState.Ready && ObjectManager.Player.Distance(target) < 600 && Config.Item("IgKs").GetValue<bool>())
